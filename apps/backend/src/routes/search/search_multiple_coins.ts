...
Router.get('/', async (req: Request, res: Response): Promise<any> => {
    ...
    try {
        ...
        return res.json(new BackendReturn(200, true, `The coinData was found successfully!`, { data: allCoinsData }))

    } catch (error) {
        return res.json(new BackendReturn(500, false, `The coinData was not found due to some server error!`, {}))
    }
})
